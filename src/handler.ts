...
function findByLongitudeLatitude(
    ...
    return dataToReturn.filter(json => !_.isEqual(json, JsonModel.EMPTY_JSON_MODEL));
}
...
function fuzzySearch(details: Details) {
    ...
    const findByFuzzySearch = [...new Set<JsonModel>([...searchByZip, ...searchByCountry, ...searchByCity])];
